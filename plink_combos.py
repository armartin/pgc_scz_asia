...
def main(args):
    ...
    for i in range(len(file_paths)):
        ...
        out.write('plink --meta-analysis \\\n')

        for j in range(len(file_paths)):
            if i != j:
                out.write(file_paths[j] + ' \\\n')
                #write_cohort_info(out, file_paths[j])

        out.write('+ report-all \\\n')
        out.write('--out /home/armartin/pgc_scz_asia/imp_collect/prs_score/armartin/cross_pop/eur_loometa/loo_' +
                  file_shorthand[i] + '\n')
        out.close()
